...
const SuggestedAccounts: NextPage<IProps> = ({ fetchAllUsers, allUsers }) => {    useEffect(() => {
      fetchAllUsers();
    }, [fetchAllUsers]);
        ...
        <div>
          {users?.slice(0, 6).map((user: IUser) => (
            <Link href={`/profile/${user._id}`} key={user._id}>
              <div className='flex gap-3 dark:hover:bg-gray-800 hover:bg-primary p-2 cursor-pointer font-semibold rounded'>
